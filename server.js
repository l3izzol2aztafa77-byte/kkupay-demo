...
app.post('/api/payments/banking', authenticate, async (req,res) => {
  try {
    ...
    res.json({ success:true, orderId, amount:parseFloat(amount), bankName:bank.name, bankShort:bank.short, accountNumber:bank.account, accountName:bank.acct_name, color:bank.color, textColor:bank.textColor, ref1:orderId, ref2, expiresAt:new Date(Date.now()+24*60*60*1000).toISOString(), steps:[`เปิดแอป ${bank.short} หรือ Internet Banking`,`เลือก "โอนเงิน" → กรอกเลขบัญชี ${bank.account}`,`ชื่อบัญชี: ${bank.acct_name}`,`จำนวน: ฿${parseFloat(amount).toFixed(2)}`,`ระบุ Ref1: ${orderId} ในช่องหมายเหตุ`,`ระบุ Ref2: ${ref2} ในช่องหมายเหตุ 2`,'กดยืนยัน แล้วเก็บสลิปไว้เป็นหลักฐาน'], instruction:'ระบบยืนยันอัตโนมัติภายใน 1-2 ชม. กรุณาระบุ Ref ให้ถูกต้องทั้ง 2 ช่อง' });
  } catch(e) { res.status(500).json({ error:e.message }); }
});
